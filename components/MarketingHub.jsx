        ...
        <div className="body-row">
          ...
          <main className={`main ${notesOpen ? "nr" : ""}`}>
            ...
            {leftTab === "timeline" && !activeBrand && (
              <NativeProjectTimelinePanel items={timelineItems} setItems={setTimelineItems} canEdit={canEdit} canAddContent={canAddContent} currentUser={currentUser} initiatives={initiatives} campaigns={campaigns} />
            )}
            ...
            {leftTab === "company" && !activeBrand && (<>
              ...
              {/* GANTT */}
              {view === "timeline" && <NativeProjectTimelinePanel items={timelineItems} setItems={setTimelineItems} canEdit={canEdit} canAddContent={canAddContent} currentUser={currentUser} initiatives={initiatives} campaigns={campaigns} />}
            </>)}
            </div>
          </main>
...
/* ─── NATIVE PROJECT TIMELINE ─── */
function NativeProjectTimelinePanel({ items, setItems, canEdit, canAddContent, currentUser, initiatives, campaigns }) {
  const [selected, setSelected] = useState(null);
  const [showAdd, setShowAdd] = useState(false);
  const [filter, setFilter] = useState("all");
  const dragRef = useRef(null);

  // 9-month rolling window: -1 to +8 months (same range as original)
  const now = new Date();
  const months = [];
  for (let m = -1; m < 8; m++) {
  ...
  const startDrag = (e, id, handle) => {
    e.preventDefault();
    e.stopPropagation();
    const track = e.currentTarget.closest(".ptl-track");
    if (!track) return;
    const rect = track.getBoundingClientRect();
  ...
  const FL = { fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--text-muted)", fontWeight: 600, marginBottom: 6, display: "block" };
  const FI = { width: "100%", padding: "7px 10px", borderRadius: 7, border: "1px solid var(--border)", background: "var(--surface2)", color: "var(--text)", fontSize: 12, fontFamily: "var(--bf)", boxSizing: "border-box" };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 57px)", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 18px", borderBottom: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
          ...
          <button className="btn btn-sm" onClick={() => window.open("/project-timeline", "_blank", "width=1500,height=820,resizable=yes,scrollbars=yes")}>↗ Pop Out</button>
          {canModify && <button className="btn btn-sm" style={{ borderColor: "rgba(201,168,76,.3)", color: "var(--gold)", fontWeight: 600 }} onClick={() => setShowAdd(true)}>+ Add Item</button>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid var(--border)", flexShrink: 0, background: "rgba(255,255,255,.02)" }}>
            <div style={{ width: 280, flexShrink: 0, padding: "9px 16px", fontSize: 10, color: "var(--text-muted)", letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600, borderRight: "1px solid var(--border)" }}>Item</div>
            <div style={{ flex: 1, display: "flex" }}>
              {months.map((m, i) => (
                <div key={i} style={{ flex: 1, textAlign: "center", padding: "9px 0", fontSize: 10, color: m.isCurrent ? "var(--gold)" : "var(--text-muted)", fontWeight: m.isCurrent ? 700 : 500, letterSpacing: ".06em", borderRight: i < months.length - 1 ? "1px solid rgba(255,255,255,.04)" : "none" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {filtered.length === 0 && (
              <div style={{ padding: "80px 0", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: "var(--text-muted)" }}>No items{filter !== "all" ? " matching filter" : " yet"}</div>
                {filter === "all" && canModify && <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 4 }}>Click + Add Item to get started</div>}
              </div>
            )}
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} onClick={() => setSelected(s => s === item.id ? null : item.id)}
                  style={{ display: "flex", borderBottom: "1px solid var(--border2)", cursor: "pointer", background: isSelected ? "rgba(201,168,76,.04)" : "transparent", transition: "background .1s", minHeight: 48 }}
                  ...
                  {/* Label col */}
                  <div style={{ width: 280, flexShrink: 0, padding: "10px 16px", borderRight: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10, minHeight: "inherit" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}>{item.category} · {item.owner || "Unassigned"}</div>
                    </div>
                  </div>
                  {/* Bar track */}
                  <div className="ptl-track" style={{ flex: 1, position: "relative", minHeight: "inherit", overflow: "hidden" }}>
                    {/* Month grid */}
                    ...
                    {item.startDate && item.endDate && (
                      <div style={{ position: "absolute", left: `${lp}%`, width: `${wp}%`, top: "50%", transform: "translateY(-50%)", height: 22, borderRadius: 5, background: `linear-gradient(90deg,${sc}55,${sc}99)`, border: `1px solid ${sc}77`, display: "flex", alignItems: "center", userSelect: "none", zIndex: 2 }}>
                    ...
                    {/* Date label */}
                    {item.startDate && item.endDate && (
                      <div style={{ position: "absolute", bottom: 2, left: `${lp}%`, fontSize: 9, color: "var(--text-muted)", pointerEvents: "none", whiteSpace: "nowrap", paddingLeft: 4 }}>
        ...
        {/* Detail panel */}
        {selectedItem && (
          <div style={{ width: 360, flexShrink: 0, borderLeft: "1px solid var(--border)", display: "flex", flexDirection: "column", background: "var(--surface)", overflowY: "auto" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text)", lineHeight: 1.3 }}>{selectedItem.title}</div>
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>{selectedItem.category} · Added by {selectedItem.createdBy || "Team"}</div>
              </div>
              <button onClick={() => setSelected(null)} style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid var(--border)", background: "transparent", color: "var(--text-muted)", cursor: "pointer", fontSize: 14, display: "grid", placeItems: "center", flexShrink: 0 }}>×</button>
            </div>
            <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  ...
                  {canModify ? <input type="date" value={selectedItem.startDate || ""} onChange={e => updateItem(selectedItem.id, { startDate: e.target.value })} style={FI} />
                    : <span style={{ fontSize: 12, color: "var(--text)" }}>{selectedItem.startDate || "—"}</span>}
                </div>
                <div>
                  <label style={FL}>End Date</label>
                  {canModify ? <input type="date" value={selectedItem.endDate || ""} onChange={e => updateItem(selectedItem.id, { endDate: e.target.value })} style={FI} />
              ...
              <div>
                ...
                {canModify ? <input value={selectedItem.owner || ""} onChange={e => updateItem(selectedItem.id, { owner: e.target.value })} placeholder="Assign owner…" style={FI} />
                  : <span style={{ fontSize: 12, color: "var(--text)" }}>{selectedItem.owner || "Unassigned"}</span>}
              </div>
              {/* Cost */}
              <div>
                <label style={FL}>Estimated Cost</label>
                {canModify ? <input value={selectedItem.cost || ""} onChange={e => updateItem(selectedItem.id, { cost: e.target.value })} placeholder="$0.00" style={{ ...FI, color: "var(--gold)", fontSize: 13, fontWeight: 600 }} />
                  : <span style={{ fontSize: 13, color: "var(--gold)", fontWeight: 600 }}>{selectedItem.cost || "—"}</span>}
              </div>
              {/* Description */}
              <div>
                <label style={FL}>Description</label>
                {canModify ? <textarea value={selectedItem.description || ""} onChange={e => updateItem(selectedItem.id, { description: e.target.value })} placeholder="Add details…" rows={3} style={{ ...FI, resize: "vertical", lineHeight: 1.65 }} />
